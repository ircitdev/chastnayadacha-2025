...
const PAGE = process.argv[2]; // relative path like 'dacha-map/otkrytye-besedki'
...
for (const img of [...imgs].sort()) {
  ...
  if (fullName !== basename) {
    const src = backupIndex[fullName] || null;
    const staticSrc = path.join(STATIC_UP, fullName);
    if (src) {
      fs.mkdirSync(path.dirname(norm), {recursive: true});
      fs.copyFileSync(src, norm);
    ...
    if (fs.existsSync(staticSrc)) {
      fs.mkdirSync(path.dirname(norm), {recursive: true});
      fs.copyFileSync(staticSrc, norm);
