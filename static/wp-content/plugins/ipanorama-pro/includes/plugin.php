...
class iPanorama_App {
	...
	function run() {
		...
		if(function_exists('register_block_type')) {
			register_block_type('ipanorama/shortcode-block', array(
				...
				'editor_script' => IPANORAMA_PLUGIN_NAME . '-gutenberg-block-js',
	...
	function shortcode($atts) {
		extract(shortcode_atts(array('id'=>0, 'slug'=>NULL, 'class'=>NULL, 'sceneid'=>NULL, 'width'=>NULL,'height'=>NULL, 'customdata'=>NULL), $atts));
	...
	*/
    ...
    function admin_footer() {
        ...
        wp_enqueue_style(IPANORAMA_PLUGIN_NAME . '-feedback-css', $plugin_url . 'assets/css/feedback.min.css', array(), IPANORAMA_PLUGIN_VERSION);
        wp_enqueue_script(IPANORAMA_PLUGIN_NAME .  '-feedback-js', $plugin_url . 'assets/js/feedback.min.js', array('jquery'), IPANORAMA_PLUGIN_VERSION, false);
        ...
        // set global settings
        wp_localize_script(IPANORAMA_PLUGIN_NAME .  '-feedback-js', 'ipanorama_feedback_globals', $globals);
    }
	...
	function block_editor_assets() {
		...
		wp_enqueue_script(IPANORAMA_PLUGIN_NAME . '-gutenberg-block-js', $plugin_url . 'gutenberg/block.min.js', array('wp-blocks', 'wp-element', 'wp-components', 'wp-editor'), IPANORAMA_PLUGIN_VERSION);
		wp_enqueue_style(IPANORAMA_PLUGIN_NAME . '-gutenberg-block-css', $plugin_url . 'gutenberg/block.min.css', array('wp-edit-blocks'), IPANORAMA_PLUGIN_VERSION);
	}
	...
	function rest_api_init() {
		register_rest_route('ipanorama/v1', '/items/', array(
			'methods' => 'GET',
	...
	function ajax_settings_update() {
		...
		if(check_ajax_referer(IPANORAMA_PLUGIN_NAME . '_ajax', 'nonce', false)) {
			...
			if($type == 'config') {
				$key = IPANORAMA_PLUGIN_NAME . '_settings';
