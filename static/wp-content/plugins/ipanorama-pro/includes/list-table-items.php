		...
		if(current_user_can('manage_options') || ($role && $role['state'] == 'all')) { // by default, the manage_options permission is only given to 'Super Users' and 'Administrators'
			...
			if($role) {
				...
				if(count($users) > 0) {
					...
					$this->count_all = $wpdb->query(sprintf('SELECT id FROM %1$s WHERE author IN (%2$s) AND NOT deleted', $table, $users));
					$this->count_mine  = $wpdb->query(sprintf('SELECT id FROM %1$s WHERE author=%2$s AND NOT deleted', $table, $current_user));
					$this->count_trash = $wpdb->query(sprintf('SELECT id FROM %1$s WHERE author IN (%2$s) AND deleted', $table, $users));
